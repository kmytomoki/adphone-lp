...
const scenarios = [
  {
    ...
    solution: "AdPhoneノードがLoRa(920MHz)で中継し、安否情報をバケツリレーで搬送する。",
...
export default function UseCasesPage() {
  return (
    <main className="flex-1">
      <PageHero
        eyebrow="USE CASES"
        title="災害シナリオ別に、AdPhoneの実運用を可視化"
        lede="DCON資料のシナリオをもとに、通信断下での実際の運用フローを「課題 → 解決 → 効果」で整理しています。"
      />

      <section className="px-6 py-14 md:px-12">
        <div className="mx-auto flex max-w-6xl flex-wrap gap-3">
          {["地震", "豪雨・水害", "孤立集落", "避難所運営"].map((tag) => (
            <span key={tag} className="border border-line bg-white px-4 py-2 text-sm text-ink-soft">
              {tag}
            </span>
          ))}
        </div>
      ...
      <section className="px-6 pb-20 md:px-12">
        <div className="mx-auto max-w-6xl space-y-6">
          {scenarios.map((item, index) => (
            <article key={item.title} className="grid gap-6 border border-line-soft bg-white p-6 md:grid-cols-2 md:p-8">
              <div className={index % 2 === 0 ? "order-1" : "order-2 md:order-1"}>
                <p className="mono mb-2 text-[11px] tracking-[0.2em] text-brand-accent">SCENARIO 0{index + 1}</p>
              ...
              <div className={index % 2 === 0 ? "order-2" : "order-1 md:order-2"}>
                <div className="paper-grid h-full min-h-72 border border-line-soft bg-paper" />
              </div>
            </article>
          ))}
        </div>
